...
% Fonts
\usepackage{fontspec}
\defaultfontfeatures{Mapping=tex-text} % allow for old latex style macros

\usepackage[math-style=ISO,bold-style=ISO,sans-style=italic,vargreek-shape=TeX]{unicode-math}
\usepackage{lualatex-math}
\setmainfont[Ligatures=TeX]{Cambria}
\setsansfont{Cambria} % This is not a sans font...
\setmonofont{Menlo}
...
\begin{pgfonlayer}{foreground}
  ...
  % Signal decay
  \node[meson_sig,color=ClrSigMeson,fill=ClrSigMeson,text=ClrTxt] (SigJpsi) at (7.8,+2.25) {$J\!/\!\psi$}  ;
  \node[meson_sig,color=ClrSigMeson,fill=ClrSigMeson,text=ClrTxt] (SigKS) [below=of SigJpsi] {$K^{\ast 0}$}  ;
  ...
  \node[meson_tag] (SSpip) at (3.3,+2.7) {$\pi^{+}$}; 
...
\begin{pgfonlayer}{foreground}
  ...
  \node[tagger_label,align=left] (SSpip_label) [right=of SSpip] {SS pion (BDT)\\ SS kaon (nnet)(for $\mathsfit{ B_s^{0}}$)};
  \node[tagger_label,align=left] (OSlepton_label) [right=of OSlepton] {OS muon\\ OS electron};
  \node[tagger_label,align=left] (OSkaon_label)   [right=of OSkaon] {OS kaon (nnet)};
  \node[tagger_label,align=left] (OScharm_label)  [above=of OS_TV] {\textcolor{red}{OS charm}};
